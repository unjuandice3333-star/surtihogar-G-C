...
const dom = new JSDOM('<!DOCTYPE html><html><body><div id="app"></div><select id="modal-category-select"></select></body></html>', { url: 'http://localhost', runScripts: 'dangerously' });
...
scriptContent = scriptContent.replace(/const supabase = createClient\(.*?\);/g, 'const supabase = window.supabase;');
